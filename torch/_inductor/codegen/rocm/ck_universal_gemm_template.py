...
class CKGemmTemplate(CKTemplate):
        ...
        if kBatch > 1:
            if op.block_gemm_pipeline_version != "BlockGemmPipelineVersion::v1":
                try:
                    ...
                    log.debug(
                        f"Failed to prefetch_stages for {op.name()} with exception {e}"
                    )
                ...
                if num_k_loop <= prefetch_stages:
                    log.debug(
                        f"Op {op.name()} is not compatible due to invalid number of pipeline prefetch stages. "
                        f"Parameters: kBatch={kBatch}, block_gemm_pipeline_version={op.block_gemm_pipeline_version}, "
                        f"prefetch_stages={prefetch_stages}, num_k_loop={num_k_loop}"
                    )
                    return False
    ...
    def _prefetch_stages(self, op, a_dtype_size, b_dtype_size, warp_size: int = 64):
        ...
        if stages is None:
            ...
            # See https://github.com/ROCm/composable_kernel/blob/d6a4605e1c77d73f5387bb4e5735411dafd6fb83/include/ck/tensor_operation/gpu/block/blockwise_gemm_pipeline_xdlops_v2.hpp#L143
