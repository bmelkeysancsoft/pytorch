...
def optimus_opportunity_finder_passes(graph: torch.fx.Graph, pre_grad: bool=False) -> None:
    log.debug("=====================================================")
    ...
    log.debug(
        print("Optimus Opportunity Finder found %s call_function and nodes." % len(item_counter))
    )
    keyword = "example_value" if pre_grad else "val"
    for target in item_counter:
        log.debug("Analysis for %s. Found %s in the graph." % (target, item_counter[target]))
        ...
        for subset in find_independent_subset_greedy(
            ...
            log.debug(
                "Find horizontal fusion opportunies. Can fuse %s node shapes." % len(subset_nodes_shape_counter)
            )
